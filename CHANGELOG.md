...
* **rack elevation generator**: a **Generate Rack Elevation** action in the node editor lays out a rack of *different* devices by rack unit (U) — define each device's label, U position, U height, and optional port count, and it places them by U (a 2U device drawn taller), drawing each device's port faceplate via the port grid engine and coloring everything green-up / red-down. An optional **rack chassis** frames it like a real rack elevation: an outer enclosure with two mounting rails, U-numbers down the left rail, and uniform full-width device bars — while the port faceplates stay labeled status nodes to the right. A companion to the port grid generator and a visualization helper only (it computes geometry from the U slots you give it, not your inventory). Ships with *Rack Elevation (generated)* and *Data Center (rack + port board)* demo dashboards ([#319](https://github.com/allamiro/grafana-network-weathermap-ng/issues/319))

* **port grid generator**: a **Generate Port Grid** action in the node editor creates a whole block of aligned port nodes in one step — for switch faceplates, patch panels, PDU strips, or blade chassis. One-click **presets** (48-/24-port switch, 24-port patch panel, PDU strip, blade chassis) fill the form, then adjust count, rows/columns, fill order (row-major / column-major / **odd-even faceplate**), a `{n}` label pattern, start number, node size, spacing, and origin; an optional status-query template (`{n}` / `{label}`) binds a query to every port at once, and **status coloring** stamps the standard green-up / red-down convention onto every port so the block reads as a live port board. An optional **block gap** breaks a board into faceplate blocks (like real switch hardware) and a shared **icon** can be drawn on every port. Generated ports are plain, editable nodes appended in a single undo step, and snap to the panel grid when snapping is on. Ships with a *Switch Port Board (generated)* demo dashboard ([#267](https://github.com/allamiro/grafana-network-weathermap-ng/issues/267))
...
* **current-version partial configs no longer crash** — a hand-edited or provisioned map with `version: 14` but missing nested settings (or missing renderer-required leaves like `panel.backgroundColor`, `panel.zoomScale`, stroke/label colors) now repairs through the defaults deep-merge in both the panel and the editor ([#224](https://github.com/allamiro/grafana-network-weathermap-ng/issues/224), [#232](https://github.com/allamiro/grafana-network-weathermap-ng/issues/232), PRs [#228](https://github.com/allamiro/grafana-network-weathermap-ng/pull/228), [#234](https://github.com/allamiro/grafana-network-weathermap-ng/pull/234))
* **all editor and interaction updates are immutable** — pan/drag persistence, node selection state, VIA insert/remove, and every form handler (116 sites) now deliver new object references instead of mutating the saved options in place; the form test harnesses deep-freeze `props.value` so the class stays closed ([#238](https://github.com/allamiro/grafana-network-weathermap-ng/issues/238), PR [#239](https://github.com/allamiro/grafana-network-weathermap-ng/pull/239)) ([#225](https://github.com/allamiro/grafana-network-weathermap-ng/issues/225), [#233](https://github.com/allamiro/grafana-network-weathermap-ng/issues/233), PRs [#229](https://github.com/allamiro/grafana-network-weathermap-ng/pull/229), [#235](https://github.com/allamiro/grafana-network-weathermap-ng/pull/235))
* **no more React NaN warnings from blanked threshold fields** — non-finite drafts render as an empty input ([#226](https://github.com/allamiro/grafana-network-weathermap-ng/issues/226), PR [#230](https://github.com/allamiro/grafana-network-weathermap-ng/pull/230))
...
* E2E now also runs on pull requests that touch the E2E surface (`tests/**`, Playwright config, the workflow) while staying weekly/manual for ordinary PRs ([#227](https://github.com/allamiro/grafana-network-weathermap-ng/issues/227), PR [#231](https://github.com/allamiro/grafana-network-weathermap-ng/pull/231))
...
* **multi-datasource demo & docs** — the `testing/` stack now runs the same simulated WAN metrics through **Prometheus, InfluxDB, Elasticsearch, and a full Zabbix 7.0 server simultaneously** (one bridge, identical series names), with per-source *WAN Demo — Utilization* dashboards rendering the byte-identical map; a new [Data Sources](https://allamiro.github.io/grafana-network-weathermap-ng/guide/datasources/) docs page covers per-source queries, the display-name binding rule, every deviation discovered (Influx `value <series>` naming, the Elasticsearch 65536-bucket limit, Zabbix data alignment and API memory/trends), and a live screenshot per source; frame-shape unit tests lock value/name resolution for each source's real frames ([#253](https://github.com/allamiro/grafana-network-weathermap-ng/issues/253), [#256](https://github.com/allamiro/grafana-network-weathermap-ng/issues/256), PRs [#254](https://github.com/allamiro/grafana-network-weathermap-ng/pull/254), [#255](https://github.com/allamiro/grafana-network-weathermap-ng/pull/255), [#257](https://github.com/allamiro/grafana-network-weathermap-ng/pull/257), [#258](https://github.com/allamiro/grafana-network-weathermap-ng/pull/258), [#259](https://github.com/allamiro/grafana-network-weathermap-ng/pull/259), [#261](https://github.com/allamiro/grafana-network-weathermap-ng/pull/261))
* dependency security — `npm audit` is clean: the dompurify override tracks the patched `^3.4.11` line, js-yaml DoS advisories resolved in both dependency paths, and the transitive `@opentelemetry/core`/`protobufjs` advisories fixed without bumping the `@grafana/*` packages (PRs [#237](https://github.com/allamiro/grafana-network-weathermap-ng/pull/237), [#242](https://github.com/allamiro/grafana-network-weathermap-ng/pull/242))
