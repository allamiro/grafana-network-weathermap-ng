...
*The WAN demo with three polyline links: the DFW site link curving out to the right so it stays clear of EDGE-1's other downlink, the NYC link arced through two bends tracing a geographic-style path, and the INET uplink routed around the top-right corner — each still one logical link with its own queries, labels, and arrows. The two CORE trunks alongside them use [Link Offset](#parallel-links-link-offset) rather than waypoints, which is usually the right tool for a pair that shares both endpoints. Try it locally with the `WAN Demo — Polyline Links` dashboard in the [testing environment](https://github.com/allamiro/grafana-network-weathermap-ng/tree/main/testing).*
